...
public class ExecutorServlet extends LoginAbstractAzkabanServlet {
    ...
    private static final String API_CANCEL_FLOW_JOBS = "cancelFlowJobs";
    ...
    private static final String API_EXECUTE_FLOW_JOBS = "executeFlowJobs";
    ...
    private static List<AzkabanAPI> createAPIEndpoints() {
        ...
        // 新添加的执行指定有效的job
        apiEndpoints.add(new AzkabanAPI("ajax", API_EXECUTE_FLOW_JOBS));
        // 新添加的停止指定的jobs
        apiEndpoints.add(new AzkabanAPI("ajax", API_CANCEL_FLOW_JOBS));
    ...
    private void handleAJAXAction(final HttpServletRequest req, final HttpServletResponse resp,
        ...
        if (hasParam(req, "execid")) {
            ...
            if (exFlow == null) {
                ...
                // 停止执行某个Flow下的部分jobs
                else if (API_CANCEL_FLOW_JOBS.equals(ajaxName)) {
                    ajaxCancelFlowJobs(req, resp, ret, session.getUser(), exFlow);
            ...
            } else if (API_EXECUTE_FLOW_JOBS.equals(ajaxName)) {
                ajaxExecuteFlowJobs(req, resp, ret, session.getUser());
            }
     ...
     * @throws ServletException
     */
    private void ajaxCancelFlowJobs(final HttpServletRequest req, final HttpServletResponse resp,
                                    final HashMap<String, Object> ret, final User user, final ExecutableFlow exFlow)
            throws ServletException {
        final Project project =
                getProjectAjaxByPermission(ret, exFlow.getProjectId(), user,
        ...
        // 获取到传入的需要停止的 jobs
        String jobs = getParam(req, "jobs");
        String[] split = jobs.split(",");
        if (split.length == 0) {
            ret.put("error", "please check param jobs , it must not be empty!");
            return;
        }
        ArrayList<String> jobLists = new ArrayList<>(Arrays.asList(split));

        // 获取当前flow下的任务的名称集合
        ArrayList<String> jobNames = new ArrayList<>();

        for (String job:jobLists) {
            logger.warn("jobLists----" +job);
        }

        for (Node allNode : project.getFlow(exFlow.getFlowId()).getNodes()) {
        ...
        if (!jobNames.containsAll(jobLists)) {
            ret.put("error", "Some jobs that need to be cancelled are not in the original job list, please check!");
            return;
        }
        ...
        try {
            ...
            // todo
            this.executorManagerAdapter.cancelFlowJobs(exFlow, jobs, user.getUserId());
    ...
    /**
     * 增加API调用方式启动对应的 project 下的 flow 下的 某些 jobs
     *
     * @param req
     * @param resp
     * @param ret
     ...
     * @author neil
     */
    private void ajaxExecuteFlowJobs(final HttpServletRequest req,
                                     final HttpServletResponse resp, final HashMap<String, Object> ret, final User user)
            throws ServletException {
        final String projectName = getParam(req, "project");
        final String flowId = getParam(req, "flow");
        final String jobIds = getParam(req, "jobIds");
        ...
        try {
            ...
            // 传入获取到的jobs
            final String message =
                    this.executorManagerAdapter.submitExecutableFlowJobs(exflow, user.getUserId(), jobIds);
            ret.put("message", message);
        } catch (final Exception e) {
            e.printStackTrace();
            ret.put("error",
