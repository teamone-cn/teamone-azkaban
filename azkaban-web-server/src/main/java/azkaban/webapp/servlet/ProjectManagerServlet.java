...
public class ProjectManagerServlet extends LoginAbstractAzkabanServlet {
    ...
    /**
     * 在当前project的flow下添加新的job
     */
    private static final String API_ADD_JOB_IN_CURRENT_FLOW = "addJobInCurrentFlow";
    /**
     * 在当前project的flow下删除jobs
     */
    private static final String API_DELETE_JOBS_IN_CURRENT_FLOW = "deleteJobsInCurrentFlow";
    /**
     * 在当前project下新增一个空的flow
     */
    private static final String API_ADD_FLOW_IN_CURRENT_PROJECT = "addFlowInCurrentProject";
    /**
     * 在当前project下删除一个flow
     */
    private static final String API_DELETE_FLOW_IN_CURRENT_PROJECT = "deleteFlowInCurrentProject";
    ...
    private static List<AzkabanAPI> createAPIEndpoints() {
        ...
        /** 在当前project的flow下添加新的job*/
        apiEndpoints.add(new AzkabanAPI("ajax", API_ADD_JOB_IN_CURRENT_FLOW));
        /** 在当前project的flow下删除jobs*/
        apiEndpoints.add(new AzkabanAPI("ajax", API_DELETE_JOBS_IN_CURRENT_FLOW));
        /** 在当前project下新增空的Flow*/
        apiEndpoints.add(new AzkabanAPI("ajax", API_ADD_FLOW_IN_CURRENT_PROJECT));
        /** 在当前project下删除一个flow*/
        apiEndpoints.add(new AzkabanAPI("ajax", API_DELETE_FLOW_IN_CURRENT_PROJECT));
    ...
    private void handleAJAXAction(final HttpServletRequest req,
        ...
        if (project == null) {
            ...
            /** 在当前project的flow下添加新的job*/
            else if (API_ADD_JOB_IN_CURRENT_FLOW.equals(ajaxName)) {
                if (uploadPrivilegeUser != null && disableJobPropsOverrideWhenProjectUploadLocked && project.isUploadLocked()) {
                    ret.put(ERROR_PARAM, "Project " + projectName + "is locked for editing job property while upload privilege user "
                            + "is set to " + uploadPrivilegeUser + ". If you really need to edit job property, "
                            + "please contact oncall to remove this lock.");
                ...
                if (handleAjaxPermission(project, user, Type.WRITE, ret)) {
                    ajaxAddJobInCurrentFlow(project, ret, req, user);
                }
                /** 在当前project的flow下删除jobs */
            } else if (API_DELETE_JOBS_IN_CURRENT_FLOW.equals(ajaxName)) {
                if (uploadPrivilegeUser != null && disableJobPropsOverrideWhenProjectUploadLocked && project.isUploadLocked()) {
                    ret.put(ERROR_PARAM, "Project " + projectName + "is locked for editing job property while upload privilege user "
                            + "is set to " + uploadPrivilegeUser + ". If you really need to edit job property, "
                            + "please contact oncall to remove this lock.");
                ...
                if (handleAjaxPermission(project, user, Type.DELETE, ret)) {
                    ajaxDeleteJobInCurrentFlow(project, ret, req, user);
                }
            } else if (API_ADD_FLOW_IN_CURRENT_PROJECT.equals(ajaxName)) {
                if (uploadPrivilegeUser != null && disableJobPropsOverrideWhenProjectUploadLocked && project.isUploadLocked()) {
                    ret.put(ERROR_PARAM, "Project " + projectName + "is locked for editing job property while upload privilege user "
                            + "is set to " + uploadPrivilegeUser + ". If you really need to edit job property, "
                            + "please contact oncall to remove this lock.");
                ...
                if (handleAjaxPermission(project, user, Type.WRITE, ret)) {
                    ajaxAddFlowInCurrentProject(project, ret, req, user);
                }
            } else if (API_DELETE_FLOW_IN_CURRENT_PROJECT.equals(ajaxName)) {
                if (uploadPrivilegeUser != null && disableJobPropsOverrideWhenProjectUploadLocked && project.isUploadLocked()) {
                    ret.put(ERROR_PARAM, "Project " + projectName + "is locked for editing job property while upload privilege user "
                            + "is set to " + uploadPrivilegeUser + ". If you really need to edit job property, "
                            + "please contact oncall to remove this lock.");
                ...
                if (handleAjaxPermission(project, user, Type.DELETE, ret)) {
                    ajaxDeleteFlowInCurrentProject(project, ret, req, user);
                }
            } else {
     ...
     * 在当前project的flow下添加新的job
     */
    private void ajaxAddJobInCurrentFlow(final Project project,
                                         final HashMap<String, Object> ret, final HttpServletRequest req, final User user)
            throws ServletException {
        final String flowName = getParam(req, "flowName");
        final Flow flow = project.getFlow(flowName);
        final Map<String, String> jobParamGroup = this.getParamGroup(req, "newJobProp");
        final Props jobParams = new Props(null, jobParamGroup);
        ...
        for (Node allNode : flow.getNodes()) {
            jobNames.add(allNode.getId());
        }
        // 判断依赖的节点是否在已有的job节点集合内
        if (StringUtils.isNotBlank(jobParams.get("dependOn"))) {
            for (String dependJobName : jobParams.get("dependOn").split(",")) {
                if (!jobNames.contains(dependJobName)) {
                    ret.put(ERROR_PARAM, "you must use the exists jobs to fill in newJobProp[dependOn]!");
                    return;
                }
            }
        }
     ...
     * 在当前project的flow下删除jobs
     */
    private void ajaxDeleteJobInCurrentFlow(final Project project,
                                            final HashMap<String, Object> ret, final HttpServletRequest req, final User user)
            throws ServletException {
        final String flowName = getParam(req, "flowName");
        final Flow flow = project.getFlow(flowName);
        final String[] deleteJobs = getParam(req, "deleteJobNames").split(",");
        ...
        // 判断传入的需要删除的jobName是不是为空
        if (deleteJobs.length == 0) {
            ret.put(ERROR_PARAM, "deleteJobNames should not be empty, please check!");
            return;
        }

        // 获取需要删除的任务的名称
        final ArrayList<String> deleteJobNames = new ArrayList<>(Arrays.asList(deleteJobs));
        ...
        // 判断已有的jobNames是否完全包含传入的jobName
        if (!jobNames.containsAll(deleteJobNames)) {
            ret.put(ERROR_PARAM, "Some jobs that need to be deleted are not in the original job list, please check!");
            return;
        }
        try {
            // 这里删除当前flow下指定的jobs
     ...
     * 在当前project下新增空的flow
     */
    private void ajaxAddFlowInCurrentProject(final Project project,
                                             final HashMap<String, Object> ret, final HttpServletRequest req, final User user)
            throws ServletException {

        // 获取当前已经存在于该project下的flow
        final List<Flow> flows = project.getFlows();

        final Map<String, String> flowParamGroup = this.getParamGroup(req, "newFlowProp");
        final Props flowParams = new Props(null, flowParamGroup);

        // 判断是否有正确传入参数
        if (ObjectUtils.isEmpty(flowParamGroup)) {
            ret.put(ERROR_PARAM, "newFlowProp should not be empty, please check!");
            return;
        }

        // 判断传入的参数是否有漏传必传参数的
        if (flowParams.get("newFlowName") == null) {
            ret.put(ERROR_PARAM,
                    "you should set newFlowProp props (newFlowName)! eg. newFlowProp[newFlowName]");
            return;
        }

        for (Flow flow : flows) {
            // 校验传入的flowName是不是已经存在于当前project下
            if (flow.getNode(flowParams.get("newFlowName")) != null) {
                ret.put(ERROR_PARAM, " flowName already exists");
                return;
            }
        }
     ...
     * 在当前project下删除flow
     */
    private void ajaxDeleteFlowInCurrentProject(final Project project,
                                                final HashMap<String, Object> ret, final HttpServletRequest req, final User user)
            throws ServletException {
        ...
        // 判断是否有正确传入参数
        if (ObjectUtils.isEmpty(flowName)) {
            ret.put(ERROR_PARAM, "flowName should not be empty, please check!");
            return;
        }

        // 判断传入的 flowName 对应的 flow 是否存在于目前的 project 中
        if (ObjectUtils.isEmpty(flow)) {
            ret.put(ERROR_PARAM, "this flow [" + flowName + "] doesn't exists in this project, please check!");
            return;
        }
    ...
    private void ajaxSetJobOverrideProperty(final Project project,
        ...
        logger.warn("overrideParams----" + overrideParams);
        logger.warn("node.getJobSource()----" + node.getJobSource());
