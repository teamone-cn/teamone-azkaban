...
public class JdbcProjectImpl implements ProjectLoader {
    ...
    @Override
    public void updateNewFlow(final Project project, final int version, final Flow flow, String name)
            throws ProjectManagerException {
        logger.info("Uploading flow " + name);
        try {
            final String json = JSONUtils.toJSON(flow.toObject());
            final byte[] data = convertJsonToBytes(this.defaultEncodingType, json);
            logger.info("Flow upload " + name + " is byte size " + data.length);
            final String UPDATE_FLOW =
                    "UPDATE project_flows SET encoding_type=?,json=? WHERE project_id=? AND version=? AND flow_id=?";
            try {
                this.dbOperator
                ...
                logger.error("Error inserting flow", e);
                throw new ProjectManagerException("Error inserting flow " + name, e);
            }
        } catch (final IOException e) {
            throw new ProjectManagerException("Flow Upload failed.", e);
        }
    ...
    @Override
    public Flow uploadNewFlow(Project project, String flowId, int flowVersion) throws ProjectManagerException {

        logger.info("Fetching " + flowId + " in project " + project.getName() + "(projectId)" + project.getId() +
                " on flow version " + flowVersion);
        ...
        try {
            data = this.dbOperator
                    .query(FlowFileResultHandler.SELECT_FLOW_FILE, handler,
                            project.getId(), project.getVersion(), flowId + ".flow", flowVersion);

            ObjectMapper objectMapper = new ObjectMapper(new YAMLFactory());

            Map<String, Object> yamlObj = (Map<String, Object>)
                    objectMapper.readValue(new ByteArrayInputStream(data.get(0)), Object.class);
            yamlObj.put("id", flowId);

            List<Object> nodes1 = (List<Object>) yamlObj.get("nodes");

            for (Object n : nodes1) {
                Map<String, Object> n1 = (Map<String, Object>) n;
                n1.put("id", n1.get("name"));
            }


            logger.info("load------" + yamlObj);

            String json = JSONUtils.toJSON(yamlObj);

            logger.info("json------" + json);

            Object flowObj = JSONUtils.parseJSONFromString(json);
            logger.info("flowObj------" + flowObj);

            Flow flow = Flow.flowFromObject(flowObj);

            logger.info("flow------" + flow);

            Map<String, Object> flowObject = (Map<String, Object>) flowObj;
            logger.info("flowObject----" + flowObject);

            String id = (String) flowObject.get("id");
            logger.info("id----" + id);

            List<Object> nodes = (List<Object>) flowObject.get("nodes");
...
//todo
//            updateNewFlow(project, project.getVersion(), flow, flowId);
...
//            JSONObject
//
////            JSONObject jsonObject = new JSONObject((JSONTokener) yamlObj);
//            System.out.println("jsonObject------" + jsonObject.toString());

//            final byte[] jsonData = convertJsonToBytes(this.defaultEncodingType, json);
//
//            logger.info("Flow upload " + flowId + " in project " + project.getName() + " is byte size " + jsonData.length);
//            final String INSERT_FLOW =
//                    "UPDATE project_flows SET json=?,modified_time=? WHERE project_id=? AND version=? AND flow_id=? ";
//            this.dbOperator
//                    .update(INSERT_FLOW, jsonData, System.currentTimeMillis(), project.getId(), project.getVersion(),
//                            flowId);

        } catch (final SQLException e) {
            logger.error("Error handling flow", e);
            throw new ProjectManagerException("Error handling flow " + flowId, e);
...
//        }
    ...
    public Map<Project, List<Flow>> fetchAllFlowsForProjects(final List<Project> projects)
        ...
        final SQLTransaction<Map<Project, List<Flow>>> transaction = transOperator -> {
            ...
            for (final Project p : projects) {
                logger.info("获取project的 projectId为" + p.getId());
                logger.info("获取project的 projectVersion为" + p.getVersion());
    ...
    @Override
    public void uploadOtherFlowFile(final int projectId, final int projectVersion, String flowName,
                                    final int flowVersion) throws ProjectManagerException {
        logger.info(String
                .format("reload all flows except for the current %s", flowName));

        final String INSERT_FLOW_FILES =
                "INSERT INTO project_flow_files " +
                        "SELECT project_id, project_version, flow_name, ? , ?, flow_file " +
                        "FROM project_flow_files " +
                        "WHERE project_id = ? AND project_version = ? AND flow_name <> ? " +
                        "AND flow_version = ?";
        try {
            this.dbOperator
                    .update(INSERT_FLOW_FILES, flowVersion, System.currentTimeMillis(), projectId, projectVersion,
                            flowName + FLOW_FILE_SUFFIX, flowVersion - 1);
        } catch (final SQLException e) {
            throw new ProjectManagerException(
                    "Error reload flow file " + flowName + ", version " + flowVersion + ".",
                    e);
        }
    }

    @Override
    public void deleteFlowInProject(final int projectId, final int projectVersion, String flowName) throws ProjectManagerException {
        logger.info(String
                .format("delete flow %s in project_flows", flowName));

        final String DELETE_FLOW_FILES =
                "delete from project_flows " +
                        "WHERE project_id = ? AND version = ? AND flow_id = ? ";
        try {
            this.dbOperator
                    .update(DELETE_FLOW_FILES, projectId, projectVersion, flowName);
        } catch (final SQLException e) {
            throw new ProjectManagerException(
                    "Error delete flow " + flowName, e);
        }
    }
    ...
    @Override
    public int getOtherLatestFlowVersion(final int projectId, final int projectVersion,
                                         final String flowName) throws ProjectManagerException {
        final IntHandler handler = new IntHandler();
        try {
            return this.dbOperator.query(IntHandler.SELECT_OTHER_LATEST_FLOW_VERSION, handler, projectId,
                    projectVersion, flowName);
        } catch (final SQLException e) {
            logger.error(e);
            throw new ProjectManagerException(
                    "Error selecting other latest flow version from project " + projectId + ", version " +
                            projectVersion + ", flow " + flowName + ".", e);
        }
    }
